...
2. `保有情報を編集` を開き、BTC/ETH/USDT/USDC の保有数量、投資信託(円)、預金(円)、小規模企業共済(円)を入力
3. 必要なら総資産目標を入力
4. 必要なら好きな資産名・円金額・カテゴリを追加
5. `保有情報を保存` を押す
6. スマホ移行時は `データ移行` からバックアップをダウンロードし、新しい端末で読み込む
7. 以後はページを開くだけで最新価格を取得して評価額を表示
...
- 保存済みデータと月次スナップショットはJSONファイルでバックアップ/復元可能
